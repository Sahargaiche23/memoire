...
import { Send, Search, Plus, Phone, Video, MoreVertical, Smile, X, Users, Heart, Share2, Paperclip, Image as ImageIcon, Download, Eye, Edit, Palette, Bell, LogOut, FileText, Link } from 'lucide-react';
...
function Messenger({ user, onLogout }) {
  ...
  const [onlineUsers, setOnlineUsers] = useState([]);  // Utilisateurs en ligne
  const [showInfo, setShowInfo] = useState(false);  // Panneau d'informations
  const [notification, setNotification] = useState(null);  // Notifications toast
  const [confirmDialog, setConfirmDialog] = useState(null);  // Modal de confirmation
  ...
  const UserAvatar = ({ user, size = 40, showOnline = false }) => {
    ...
    return (
      <div className="user-avatar-wrapper" style={{ width: size, height: size, position: 'relative' }}>
        {user?.profile_image ? (
          <img 
  ...
  const fetchConversations = async () => {
    try {
      ...
      // Récupérer les IDs des groupes pour les filtrer
      const groupIds = groups.map(g => g.id);

      const grouped = {};
      response.data.forEach(msg => {
        try {
          // FILTRER les messages de groupe de 2 façons:
          // 1. Vérifier si recipient_id est un ID de groupe
          // 2. Vérifier si subject contient "Message groupe:"
          const isGroupMessage = groupIds.includes(msg.recipient_id) || 
                                 (msg.subject && msg.subject.includes('Message groupe:'));
          
          if (isGroupMessage) {
            // Ignorer les messages de groupe dans les conversations 1-à-1
  ...
  const fetchMessages = async () => {
    try {
      ...
      // Pour les groupes: charger les messages du groupe depuis le backend
      if (selectedConversation?.type === 'group') {
        ...
        try {
          ...
          if (Array.isArray(response.data)) {
            ...
            console.log('📋 Structure des messages:', response.data.slice(0, 2).map(m => ({
              id: m.id,
  ...
  const fetchGroups = async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        // Pour chaque groupe, charger les détails des membres
        const groupsWithMembers = await Promise.all(
          response.data.map(async (g) => {
            try {
              // Charger les détails du groupe avec les membres
              const detailsResponse = await axios.get(`http://localhost:5000/api/groups/${g.id}`, {
                headers: { Authorization: `Bearer ${token}` }
              });
              ...
              return {
                ...
                membersList: detailsResponse.data.members || [] // Liste complète des membres avec photos
              };
            } catch (err) {
              console.error(`Erreur chargement membres groupe ${g.id}:`, err);
              return {
                id: g.id,
                name: g.name || 'Groupe sans nom',
                members: g.members_count || 0,
        ...
        console.log('✅ Groupes chargés avec membres:', groupsWithMembers);
      } else {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (selectedConversation?.type === 'group') {
      try {
        ...
        // Envoyer le message au backend pour le groupe
        const messageData = {
          group_id: groupId,
          content: newMessage
        };

        const response = await axios.post('http://localhost:5000/api/groups/' + groupId + '/messages', messageData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        showNotification('❌ Erreur envoi message groupe', 'error');
        return;
      }
    }
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file.type.startsWith('image/')) {
      ...
      showNotification(`❌ "${file.name}" n'est pas une image. Pour envoyer des fichiers, utilisez le bouton 📎`, 'error');
  ...
  const getConversationMessages = () => {
    ...
    if (selectedConversation?.type === 'group') {
      ...
      // TEMPORAIRE: Retourner TOUS les messages sans filtrage pour tester
      ...
      // Retourner TOUS les messages pour tester
      return messages;
      
      /* ANCIEN CODE AVEC FILTRAGE (désactivé temporairement):
      const filtered = messages.filter(msg => msg.recipient_id === groupId || msg.group_id === groupId);
      return filtered;
      */
    }
              ...
              <div className="users-list">
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map(u => (
                    ...
                    >
                      <UserAvatar user={u} size={45} showOnline={true} />
                  ...
                  {group.membersList && group.membersList.length > 0 && (
                    <div className="group-members-avatars">
                      {group.membersList.slice(0, 3).map((member, idx) => (
                        <div key={member.id} className="member-avatar-small" style={{ zIndex: 3 - idx }}>
                      ...
                      {group.membersList.length > 3 && (
                        <div className="member-avatar-small more">+{group.membersList.length - 3}</div>
                      )}
                    </div>
                  )}
          ...
          <div className="conversations-list">
            {filteredConversations.length === 0 ? (
              ...
              filteredConversations.map(conv => (
                ...
                >
                  <UserAvatar 
                    user={users.find(u => u.id === (conv.sender_id === user.id ? conv.recipient_id : conv.sender_id))} 
                    size={50} 
                    showOnline={true} 
                  />
            ...
            <>
                ...
                <div className="chat-header-info">
                  <div className="recipient-selector">
                    ...
                    <div className="recipient-info">
                      <h3>{currentRecipient.full_name || currentRecipient.name}</h3>
                      <p className="status-text">
                        {onlineUsers.includes(currentRecipient.id) ? (
              ...
              <div className="chat-messages">
                {getConversationMessages().map(msg => {
                  ...
                  // Pour les groupes, utiliser msg.sender_name de l'API
                    ...
                    <div key={msg.id} className={`message-group ${isOwnMessage ? 'own' : 'other'}`}>
                      ...
                      {shouldShowSenderInfo && (
                        <div className="message-header">
                          <UserAvatar user={senderUser} size={32} showOnline={false} />
                        ...
                        >
                            ...
                            <button 
                              ...
                              onClick={(e) => {
                                ...
                                if (fileInfo && fileInfo.data) {
                                  ...
                                  showNotification('⚠️ Fichier non disponible (ancien format)', 'error');
                                }
                              }}
                            >
                              <Download size={14} />
                          ...
                          {isOwnMessage && !isCall && (
                            <div className="message-status">
                              ...
                              <span className="message-status-icon" title="Envoyé">
                                ✓✓
                              </span>
                            </div>
                          )}
            ...
            <div className="info-panel-content">
              ...
              {selectedConversation?.type === 'group' ? (
                <>
                  ...
                  <div className="info-section">
                    <div className="info-profile">
                      <div className="group-avatar-large">
                        {selectedConversation.group.name.charAt(0).toUpperCase()}
                      </div>
                      <h2>{selectedConversation.group.name}</h2>
                      <p className="info-role">Groupe • {selectedConversation.group.members?.length || 0} membres</p>
                    </div>
                  </div>

                  {/* Add People Button */}
                  <div className="info-section">
                    <button className="info-action-btn" onClick={() => showNotification('Fonction ajout de membres à venir', 'info')}>
                      <Users size={20} />
                  ...
                  <div className="info-section">
                    ...
                    <div className="group-members-list">
                      {selectedConversation.group.members?.map(member => (
                        <div key={member.id} className="group-member-item">
                          <UserAvatar user={member} size={36} showOnline={false} />
                          <span>{member.full_name || member.name}</span>
                        </div>
                      )) || <p className="no-data">Aucun membre</p>}
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div className="info-section">
                    <div className="info-profile">
                      <UserAvatar user={currentRecipient} size={80} showOnline={false} />
                      <h2>{currentRecipient.full_name || currentRecipient.name}</h2>
                      {currentRecipient.role && (
                        <p className="info-role">{currentRecipient.role}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Actions Section */}
                  <div className="info-section">
                    <button className="info-action-btn" onClick={() => initiateCall('audio')}>
                      <Phone size={20} />
                      <span>Appeler</span>
                    </button>
                    <button className="info-action-btn" onClick={() => initiateCall('video')}>
                      <Video size={20} />
                  ...
                  <div className="info-section">
                    ...
                    {currentRecipient.email && (
                      <div className="info-detail">
                        ...
                        <span className="info-value">{currentRecipient.email}</span>
                      </div>
                    )}
                    {currentRecipient.id && (
                      <div className="info-detail">
                        <span className="info-label">ID:</span>
                        <span className="info-value">#{currentRecipient.id}</span>
                      </div>
                    )}
                  </div>
        ...
        <>
          ...
          >
            {contextMenu.type === 'group' ? (
              <>
                <button 
                  onMouseDown={(e) => { e.preventDefault(); confirmLeaveGroup(contextMenu.id); setContextMenu(null); }} 
                  className="danger"
                >
                  <span>👋</span> Quitter le groupe
                </button>
                <button 
                  onMouseDown={(e) => { e.preventDefault(); confirmDeleteConversation(contextMenu.id); setContextMenu(null); }} 
                  className="danger"
                >
              ...
              <>
                ...
                <button 
                  onMouseDown={(e) => { e.preventDefault(); showNotification('Conversation archivée', 'success'); setContextMenu(null); }}
                >
                ...
                <button 
                  onMouseDown={(e) => { e.preventDefault(); showNotification('Conversation signalée', 'success'); setContextMenu(null); }} 
                  className="danger"
                >
